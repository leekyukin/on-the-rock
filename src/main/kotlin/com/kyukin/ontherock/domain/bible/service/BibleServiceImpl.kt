...
class BibleServiceImpl constructor(
    ...
    override fun searchByWord(command: BibleCommand.SearchByWordRequest, pageable: Pageable): Page<BibleResponse.Main> {
        val biblePage = bibleReader.findBiblePage(command.word, pageable)
        return biblePage.map { bibleDtoMapper.of(it) }
    }

    override fun searchChapter(command: BibleCommand.SearchByChapterRequest, pageable: Pageable): Page<BibleResponse.Main> {
        val biblePage = bibleReader.findBiblePage(command.longLabel, command.chapter, pageable)
        return biblePage.map { bibleDtoMapper.of(it) }
    }

    override fun searchOneBible(id: Long): BibleResponse.Main {
